...
void handle_button_interrupts(void)
{
  ...
  printf("handle_button_interrupts before reg %x\n", btn_reg);
  btn_reg = btn_reg & 0xf;
  printf("handle_button_interrupts reg %x\n", btn_reg);
...
void ControlTimerTmrCallback (void *ptmr, void *callback_arg){
...
static void init_button_pio()
{
  ...
  IOWR_ALTERA_AVALON_PIO_IRQ_MASK(D2_PIO_KEYS4_BASE, 0xf);  // 3 buttons, 8,4,2
...
void StartTask(void* pdata)
{
  ...
  if (alt_alarm_start (&alarm,
        delay,
        alarm_handler,    // callback funtion, is called after delay
        context) < 0)
  {
  ...
  ControlTimer = OSTmrCreate(0, //delay
                      ...
                      OS_TMR_OPT_PERIODIC,
                      ControlTimerTmrCallback, //OS_TMR_CALLBACK
                      (void *)0,
                      "ControlTimer",
                      &err);
